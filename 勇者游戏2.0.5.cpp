...
char name[100]; 
int attack;
int defense; //勇者游戏2.0.5版本更新
int health;//更新日志：修复大部分bug，OrdinaryAct()函数的bug修复了一半（另一半不懂了） 
long int max_health;//增加角色潜水员，修改水下战斗模块 
...
key={"锁匠","\0",45,35,100,100,1,0,0,100,20,20,5},
diver={"潜水员","\0",50,40,100,100,1,0,0,100,20,20,6},
...
int cao=4,jijiubao=3,baiyao=3,superbaiyao=1,boom=4,dubiao=3,atom_boom=1,aircase=1,diangun=1,yulei=1,zidan=1,d;
...
char gongname[20]="无",fangname[20]="无",fujin1[10]="无",fujin2[10]="无",fujin3[10]="无",fujin4[10]="无",direction;
char proof;
char *p;
void AddWupin(int);
int AttackResult();
...
SlowDisplay("选择你想当的角色（1、大力士  2、投弹手  3、美女  4、狙击手  5、锁匠  6、潜水员）\n\n\n");
do
{
...
if(player.player_number==diver.player_number)player.attack/=1.5;
else player.attack*=1.5;
system("pause");
if(s<guai.wupinpro)
{
...
while(direction=='\0');
Setjindutiao(20);
if(direction=='m'||direction=='M')Menu();
else if(direction!='\0')
{
switch(place.sign)
{
...
default:if(d!=0)MessageBox(NULL,"错误!","警告",MB_OK);
}
break;
case 1:
switch(direction)
