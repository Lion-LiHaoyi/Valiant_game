...
char name[21]; 
int attack;
int defense; //勇者游戏1.6.5版本更新 
int health;//更新日志： 修复部分bug，新增电棍（带有电晕技能）、鱼雷（只能在海下使用） 
long int max_health;// 手雷毒镖核弹均不可在海下使用，增加巨兽鲨出现几率。 
int level; //（个人吐槽：陈曦把勇者游戏搞成了校园游戏） 
int exp; //此版本已经让手撕校长成为了可能，希望陈曦8月10日直接加可视化 
...
int max_exp=0;
int choose_number=0,s=0,strongman_arm=0,battle=0,money=1000,place_sign=11,bi=0,buy_number=0,yun=0;
int cao=4,jijiubao=3,baiyao=3,superbaiyao=1,boom=4,dubiao=3,atom_boom=1,aircase=1,diangun=1,yulei=1;
...
{
printf("物品: 1,止血草%d个 2,急救包%d个 3,云南白药%d个 4,超级云南白药%d个 5,手雷%d个 6,毒标%d个 7,手抛式原子弹%d个 8,逼能激光炮 9,氧气袋%d个 10,电棍%d个 11,鱼雷%d个 0,返回\n\n\n",cao,jijiubao,baiyao,superbaiyao,boom,dubiao,atom_boom,aircase,diangun,yulei);
...
else SlowDisplay("非战斗状态或不在海中,不能使用手雷!\n\n\n");
break;
case 0:break;
...
case 4:printf("要买点什么?\n\n\n 1：止血草100金币 2：急救包180金币 3：云南白药200金币 4：手雷150金币  5：氧气袋100金币  6：电棍1000金币  7：鱼雷500金币  0,返回\n\n\n");
buy_number=1;
while(buy_number)
{
